...
func TestDefer(t *testing.T) {
	...
	testCases := []struct {
		...
		f:    func(e *Catcher) { e.Defer(closer.Close, h1) },
		want: "Close",
	}, {
		f: func(e *Catcher) {
		...
		f:       func(e *Catcher) { e.Defer(locker.Unlock) },
		err:     errTest,
		wrapped: errWrap,
		want:    "Unlocked",
	}, {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.want, func(t *testing.T) {
			err := func() (err error) {
				ec := Catch(&err, tc.defHandlers...)
				defer ec.Handle()
				tc.f(&ec)
				ec.Must(tc.err, HandlerFunc(func(s State, err error) error {
